app.controller('CustomerDetailController', function($scope, $http, $stateParams, $state, $rootScope, paginate){
    $rootScope.title = "Заказчик";
    $http.get('customer/'+$stateParams.id)
        .success(function(data) {
            $scope.customer = data;
            $rootScope.title = $scope.customer.name;
    ...
    function init(order, orderby){
        ...
        $scope.url='customer/'+$stateParams.id+'/purchases?order='+$scope.order+'&orderby='
            +$scope.orderby+'&pageSize=';
